...
const FALLBACK_HOME = '#1a3a52';
const FALLBACK_AWAY = '#7a1e2c';
...
function buildGradient(homeColor, awayColor) {
  const left = homeColor || FALLBACK_HOME;
  const right = awayColor || FALLBACK_AWAY;
...
function normalizeState(data) {
  // `giocata: true` ha sempre la precedenza, anche con punteggi parziali o mancanti.
...
function enrichMatch(entry, teamLookup) {
  ...
  return {
    ...
    score_label: isPlayed ? `${formatScoreValue(homeGoals)} - ${formatScoreValue(awayGoals)}` : '',
...
function getRecommendedGiornata(matches) {
  ...
  const oggiStr = `${parts.year}-${parts.month}-${parts.day}`;
  const weekdayMap = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
  const giornoSettimana = weekdayMap[parts.weekday];
